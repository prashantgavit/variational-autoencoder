...
class ConditionalVAE(BaseVAE):
    ...
    def __init__(self,
        ...
        for h_dim in hidden_dims:
            # print('conv enc',in_channels,h_dim)
            modules.append(
                nn.Sequential(
        ...
        # for image 105
        self.decoder_input = nn.Linear(latent_dim + num_classes, hidden_dims[-1] * 16)
        ...
        # for image 105
        ops = [0, 1, 0, 0]   # instead of always output_padding=1
        for i in range(len(hidden_dims) - 1):
            modules.append(
                nn.Sequential(
                    nn.ConvTranspose2d(hidden_dims[i],
    ...
    def encode(self, input: Tensor) -> List[Tensor]:
        ...
        # print('after enoder conv', result.shape)
        result = torch.flatten(result, start_dim=1)
        # print('after encode flatten', result.shape)
    ...
    def decode(self, z: Tensor) -> Tensor:
        ...
        # print('after decoder_input',result.shape)
        # for Image 64
        # result = result.view(-1, 512, 2, 2)
        # for Image 105
        result = result.view(-1, 512, 4, 4)

        # print('after view',result.shape)
        result = self.decoder(result)
        # print('after decoder',result.shape)
        result = self.final_layer(result)
        # print('after final',result.shape)
        return result
